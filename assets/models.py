...
class Asset(models.Model):
    ...
    asset_type = models.CharField(max_length=64,default='server',verbose_name="资产类型",choices=asset_type_choice)
    name = models.CharField(max_length=64,unique=True,verbose_name="资产名称")  #不可重复
    sn = models.CharField(max_length=128,unique=True,verbose_name="序列号")    #不可重复
    status = models.SmallIntegerField(choices=asset_status,default=0,verbose_name="设备状态")   #小整数字段
    manage_ip = models.GenericIPAddressField(null=True, blank=True, verbose_name='管理IP')     #通用IP字段
    tags = models.ManyToManyField('Tag', blank=True, verbose_name='标签')                      #比如windows,linux,临时,备份等,对于多对多字段不需要null=True无效的字段)
    business = models.ForeignKey('BusinessUnit', null=True, blank=True, verbose_name="所属BU",
    ...
    admin = models.ForeignKey(User, null=True, blank=True, verbose_name='资产管理员', related_name='admin',
                                    on_delete=models.SET_NULL)
    #使用使用django内置的user表,资产刚创建可以暂时不分配管理员/人员离职等。
    ...
    is_upkeep = models.DateField(null=True,blank=True,verbose_name="是否维保",choices=upkeep)
    ...
    approved_by = models.ForeignKey(User, null=True, blank=True, verbose_name='批准人', related_name='approved_by',
                                    on_delete=models.SET_NULL)
    # 外键,批准人和管理员,批准人通常是主管,管理员则是服务器管理人员,
    # related_name 表示在同一个表对另外一个表引用引用需要设置该选项,在反向查找时就可以通过related_name进行引用
    ...
    def __str__(self):
        ...
        #get_asset_type_display() 表示出现choice字段就显示右边说明字段
...
class Server(models.Model):
    ...
    #有些服务器是虚拟机，没有物理实体，存在宿主机中，需要增加一个hostd_on字段，如果物理机删除了,虚拟机也不再了
    #服务器都有型号信息，如果硬件信息中不包含，就指定主板型号
...
class NetworkDevice(models.Model):
    ...
    asset = models.OneToOneField('Asset',on_delete=models.CASCADE)
    sub_asset_type = models.SmallIntegerField(choices=sub_asset_type_choice,default=1,verbose_name='网络设备')
...
class Software(models.Model):
    ...
    def __str__(self):
        return "{}--{}".format(self.get_sub_asset_type_display(),self.version)
...
class IDC(models.Model):
    ...
    phone = models.CharField(max_length=16,blank=True,null=True,verbose_name="机房联系人")
    def __str__(self):
        return self.name
...
class BusinessUnit(models.Model):
    ...
    name = models.CharField('BU单元',choices=unit_name,max_length=64, unique=True)
...
class NewAssetApprovalZone(models.Model):
    ...
    ram_size = models.PositiveIntegerField(blank=True, null=True, verbose_name='内存大小G')
    disk_size = models.PositiveIntegerField(blank=True,null=True,verbose_name='磁盘大小G')
